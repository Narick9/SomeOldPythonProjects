...
# Модифицированна по условию задачи №1 главы 12
...
class Debris(Collider):
    image = games.load_image("ship.bmp")
    SPEED = 0.1
    POINTS = 10

    def __init__(self, game, x, y):
        super().__init__(image = Debris.image,
              x = x, y = y,
              dx = random.choice((-1, 1)) * random.random() * Debris.SPEED,
              dy = random.choice((-1, 1)) * random.random() * Debris.SPEED)
        self.game = game
        self.clash = 0
    ...
    def die(self):
        self.clash += 1
        if self.clash >= 2:
            self.game.score.value += int(Debris.POINTS)
